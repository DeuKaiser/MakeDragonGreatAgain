...
{
    // 运行期后置保障：在所有本地化模组（如 QuickLocalization）覆盖完条目之后，
    // 如有需要通过迁移到新的唯一键，确保龙脉奥秘描述包含成长文本。
    internal class ArcanaLateDescriptionFix : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            _frames++;
            // 等待一段时间后再启动，以便其他模组完成（QuickLocalization 很早加载，但我们仍稍作等待）
            if (_frames < 300) return; // 约 5 秒
            ...
            try
            {
                ...
                foreach (var guid in ArcanaGuids)
                {
                    ...
                    try
                    {
                        ...
                        bool hasScaling = text.Contains("15级") || text.Contains("15th level");
                        if (!hasScaling)
                        {
