...
{
    // 临时调试热键：在游戏中按 F8 输出复合/天使神话法术表信息
    ...
    internal class GDTempDebug : MonoBehaviour
    {
        ...
        void Update()
        {
            try
            {
                // 先通过反射尝试旧版 Input（部分裁剪构建移除了直接引用符号）
                bool pressed = false;
                try
                {
                ...
                if (!pressed)
                {
                    // 回退：检查 Event.current（仅在 OnGUI 有效）或忽略
                }
                if (pressed) Run();
            }
            catch { }
        ...
        private static void Run()
        {
            try
            {
                var mergeGuid = BlueprintGuid.Parse("5bf6f5d4d2e04a1a9f4b4f4b6a9a1111"); // 自定义合书特性
